//! Rebrew — a small coffee machine for the desktop.
//!
...
fn icon_presets() -> Vec<Preset> {
    recipes::ICONS
        .iter()
        .map(|i| Preset { id: i.id.into(), label: i.label.into(), value: i.id.into() })
        .collect()
}
...
fn install_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let show = MenuItem::with_id(app, "show", "Show Rebrew", true, None::<&str>)?;
    let hide = MenuItem::with_id(app, "hide", "Hide Rebrew", true, None::<&str>)?;
    ...
    TrayIconBuilder::with_id(TRAY_ID)
        ...
        .on_menu_event(|app, event| match event.id.as_ref() {
            ...
            "keep" => {
                ...
                let _ = app.emit_to("main", "rebrew://always-on-top", next);
            }
            "quit" => {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while building Rebrew")
        .run(|app, event| {
